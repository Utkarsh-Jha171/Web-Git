...
class CarPreview {
  ...
  loadModel(type = 'car', key = 'red') {
    ...
    this.loader.load(path, (gltf) => {
      ...
      // === BIKE RESHAPE ===
      if (type === 'bike') {
        // Squish chassis X to create narrow bike frame
        node.scale.x = scale * 0.45;

        // Find wheels by name (car models typically name them wheel-fl/fr/bl/br)
        ...
        // Center remaining wheels on X=0 and restore wheel thickness
        ['wheel-fr', 'wheel-bl'].forEach(n => {
          const w = wheelNodes[n];
          if (w) {
            w.position.x = 0;
            w.scale.x = (w.scale.x / 0.45);
          }
        });
      }
      // === END BIKE RESHAPE ===
