...
// Control state
const keyState = {
  w: false, s: false, a: false, d: false, boost: false
};

// Boost energy system
let boostEnergy = 5.0; // seconds of boost
const maxBoostEnergy = 5.0;
const boostDrainRate = 1.0; // energy units per second while boosting
const boostRechargeRate = 0.6; // energy units per second when recharging
const boostRechargeDelay = 6.0; // seconds delay after last boost before recharge (increased)
let boostCooldown = false; // true when depleted and waiting for recharge threshold
let lastBoostUsedTime = 0; // timestamp in ms of last boost activity
let boostPermanentlyDisabled = false; // once true, boost cannot be used for the rest of the session
window.boostPermanentlyDisabled = boostPermanentlyDisabled;

// HUD elements (created at runtime)
let boostContainerEl = null;
let boostBarEl = null;
let boostTextEl = null;
...
function createRaceUI() {
  ...
  // Create boost HUD (small bar in top-right)
  boostContainerEl = document.createElement('div');
  boostContainerEl.id = 'boost-hud';
  boostContainerEl.style.position = 'absolute';
  boostContainerEl.style.top = '12px';
  boostContainerEl.style.right = '12px';
  boostContainerEl.style.width = '200px';
  boostContainerEl.style.height = '28px';
  boostContainerEl.style.borderRadius = '8px';
  boostContainerEl.style.padding = '4px';
  boostContainerEl.style.background = 'rgba(0,0,0,0.45)';
  boostContainerEl.style.display = 'flex';
  boostContainerEl.style.alignItems = 'center';
  boostContainerEl.style.gap = '8px';
  boostContainerEl.style.zIndex = '1500';

  boostTextEl = document.createElement('div');
  boostTextEl.style.color = '#fff';
  boostTextEl.style.fontFamily = "'Poppins', sans-serif";
  boostTextEl.style.fontSize = '12px';
  boostTextEl.style.fontWeight = '700';
  boostTextEl.textContent = 'BOOST';
  boostContainerEl.appendChild(boostTextEl);
  ...
  boostBarEl.style.height = '100%';
  boostBarEl.style.background = 'linear-gradient(90deg, #7cff6b, #ffd86b)';
  boostBarEl.style.transformOrigin = 'left center';
  boostBarEl.style.width = '100%';
  barWrap.appendChild(boostBarEl);
  boostContainerEl.appendChild(barWrap);

  document.body.appendChild(boostContainerEl);
}
...
function setupKeyControls() {
  document.addEventListener('keydown', (event) => {
    // Don't capture controls when typing in an input or editable element
    const active = document.activeElement;
    if (active && (active.tagName === 'INPUT' || active.tagName === 'TEXTAREA' || active.isContentEditable)) return;

    const key = event.key;
    if (key === 'w' || key === 'W' || key === 'ArrowUp') keyState.w = true;
    if (key === 's' || key === 'S' || key === 'ArrowDown') keyState.s = true;
    if (key === 'a' || key === 'A' || key === 'ArrowLeft') keyState.a = true;
    if (key === 'd' || key === 'D' || key === 'ArrowRight') keyState.d = true;

    // Boost when Shift is held (unless permanently disabled)
    if (key === 'Shift' && !boostPermanentlyDisabled) keyState.boost = true;
  ...
  document.addEventListener('keyup', (event) => {
    const key = event.key;
    if (key === 'w' || key === 'W' || key === 'ArrowUp') keyState.w = false;
    if (key === 's' || key === 'S' || key === 'ArrowDown') keyState.s = false;
    if (key === 'a' || key === 'A' || key === 'ArrowLeft') keyState.a = false;
    if (key === 'd' || key === 'D' || key === 'ArrowRight') keyState.d = false;
    if (key === 'Shift') keyState.boost = false;
  });
}
...
function animate() {
  ...
  // Boost energy and HUD update
  if (boostBarEl) {
    const now = Date.now();

    // Drain while boosting (only when race is active and not in cooldown and not permanently disabled)
    if (keyState.boost && raceState.raceStarted && !boostCooldown && !boostPermanentlyDisabled) {
      boostEnergy -= boostDrainRate * deltaTime;
      lastBoostUsedTime = now;
      if (boostEnergy <= 0) {
        // Permanently disable boost when depleted
        boostEnergy = 0;
        boostCooldown = true;
        boostPermanentlyDisabled = true;
        window.boostPermanentlyDisabled = true;
        // stop boosting when depleted
        keyState.boost = false;
      }
    } else {
      // Recharge after a short delay since last boost (only if not permanently disabled)
      if (!boostPermanentlyDisabled && lastBoostUsedTime > 0 && (now - lastBoostUsedTime) >= boostRechargeDelay * 1000) {
        boostEnergy += boostRechargeRate * deltaTime;
    ...
    // Update HUD
    const percent = boostEnergy / maxBoostEnergy;
    if (boostPermanentlyDisabled) {
      boostBarEl.style.width = `0%`;
      boostTextEl.textContent = 'BOOST DEPLETED';
      boostBarEl.style.opacity = '0.6';
      boostBarEl.style.filter = 'grayscale(80%)';
      boostBarEl.style.background = 'linear-gradient(90deg, #ff6b6b, #ff8b6b)';
    } else {
      boostBarEl.style.width = `${Math.round(percent * 100)}%`;
      boostTextEl.textContent = `BOOST ${Math.round(percent * 100)}%`;
      boostBarEl.style.opacity = '1';
      boostBarEl.style.filter = 'none';
      boostBarEl.style.background = 'linear-gradient(90deg, #7cff6b, #ffd86b)';
    }
  }
...
function createMobileControls() {
  ...
  // Create boost button for mobile (maps to Shift)
  ...
  boostButton.style.width = '96px';
  boostButton.style.height = '96px';
  boostButton.style.borderRadius = '50%';
  boostButton.style.backgroundColor = '#ff0080';
  boostButton.style.color = '#fff';
  boostButton.style.fontWeight = '700';
  boostButton.style.fontSize = '16px';
  boostButton.style.border = '2px solid #b30059';
  boostButton.style.boxShadow = '0 6px 0 #b30059';
  boostButton.style.display = 'none'; // shown only on mobile
  boostButton.style.zIndex = '1000';
  boostButton.style.touchAction = 'none';
  document.body.appendChild(boostButton);

  // Boost press handlers (mobile) - respect permanent disable
  const boostPress = (e) => { if (!boostPermanentlyDisabled) keyState.boost = true; e && e.preventDefault && e.preventDefault(); };
  const boostRelease = (e) => { keyState.boost = false; };
  boostButton.addEventListener('touchstart', boostPress);
  boostButton.addEventListener('mousedown', boostPress);
  boostButton.addEventListener('touchend', boostRelease);
  boostButton.addEventListener('touchcancel', boostRelease);
  boostButton.addEventListener('mouseup', boostRelease);
  boostButton.addEventListener('mouseleave', boostRelease);
  ...
  function checkMobile() {
    const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) || window.innerWidth < 768;
    joystickContainer.style.display = isMobile ? 'block' : 'none';
    // Also show boost button on mobile
    if (boostButton) boostButton.style.display = isMobile ? 'block' : 'none';
  }
